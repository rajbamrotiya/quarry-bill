...
@php
    $finalSupplierName = $buy_receipt ? $buy_receipt->supplier->name : $supplierName;
    $finalVehicleNumber = $buy_receipt ? $buy_receipt->vehicle_number : $vehicleNumber;
    $finalMaterialName = $buy_receipt ? $buy_receipt->materialType->name : $materialName;
    $finalDate = $buy_receipt ? $buy_receipt->date->format('Y-m-d') : $date;
    $finalTime = $buy_receipt ? $buy_receipt->time : $time;
    $finalGross = $buy_receipt ? $buy_receipt->gross_weight : $gross;
    $finalTare = $buy_receipt ? $buy_receipt->tare_weight : $tare;
    $finalNet = $buy_receipt ? $buy_receipt->net_weight : $net;
    $finalRemarks = $buy_receipt ? $buy_receipt->remarks : $remarks;
    $finalSlipNumber = $buy_receipt ? ($buy_receipt->pass_number ?: '#' . str_pad($buy_receipt->id, 10, '0', STR_PAD_LEFT)) : $slipNumber;
@endphp

<div class="buy_receipt-box {{ $copyClass }} {{ $preview ? 'preview-mode' : '' }}">
    ...
    <table class="data-table">
        <tr>
            <td>
                ...
                <p class="field-value">{{ $finalSupplierName ?: '---' }}</p>
            </td>
            <td>
